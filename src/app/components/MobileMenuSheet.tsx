...
export function MobileMenuSheet({ isLoggedIn, onNavigate, onLogout, onClose }: MobileMenuSheetProps) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="flex flex-col h-full justify-center items-center p-8 text-center">
        ...
        <h2 className="text-xl font-bold mb-2">{t('auth.welcomeGuest')}</h2>
        <p className="text-gray-600 mb-6">{t('auth.guestDesc')}</p>
        <button
  ...
  const menuItems = [
    ...
    { icon: HelpCircle, label: t('profile.helpSupport'), page: 'home' as Page }, // Placeholder
  ];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-bold">Sarah Ahmed</h2>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center justify-between gap-4 p-4 hover:bg-gray-50 transition-colors"
              >
                <div className="flex items-center gap-4">
